...
async function main() {
    ...
    const mockDAO = '0x436aD3b30B2BaB1bCba0a557d8812a58B6fbcFa8';
    ...
    const daiBond = await DAIBond.deploy(ohm.address, dai.address, treasury.address, mockDAO, zeroAddress);

    // Deploy Frax bond
    //@dev changed function call to Treasury of 'valueOf' to 'valueOfToken' in BondDepository due to change in Treausry contract
    // const FraxBond = await ethers.getContractFactory('MockOlympusBondDepository');
    // const fraxBond = await FraxBond.deploy(ohm.address, frax.address, treasury.address, mockDAO, zeroAddress);
